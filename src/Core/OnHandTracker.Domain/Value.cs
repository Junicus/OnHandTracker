...
namespace OnHandTracker.Domain
{
    public abstract class Value<T> where T : Value<T>
    {
        ...
        public override int GetHashCode() => CombineHashCodes(Members.Select(m =>
        ...
        public override string ToString()
        {
            ...
            {
                var value = m.GetValue(this);
                return m.IsNonStringEnumerable ? $"{m.Name}:{string.Join("|", GetEnumerableValues(value))}" :
                    m.Type != typeof(string) ? $"{m.Name}:{value}" :
                    value == null ? $"{m.Name}:null" : $"{m.Name}:\"{value}\"";
            });
        ...
        private static int CombineHashCodes(IEnumerable<object> objs)
        {
            unchecked
            {
                return objs.Aggregate(13, (current, obj) => current * 32 + (obj?.GetHashCode() ?? 0));
            }
        }
